...
def generate_password_list(complexity, length, num_passwords):
    ...
    while len(password_list) < num_passwords:
        ...
        password_list.add((password, sha256_password))  # Almacenar la contraseña en claro y su hash
...
for complexity in complexities:
    for length in password_lengths:
        clear_file_name = f'{complexity}_len{length}_clear_SHA256.txt'
        hash_file_name = f'{complexity}_len{length}_hashes_SHA256.txt'
