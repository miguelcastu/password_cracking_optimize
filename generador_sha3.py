...
def generate_password_list(complexity, length, num_passwords):
    ...
    while len(password_list) < num_passwords:
        ...
        sha3_password = hashlib.sha3_256(password.encode()).hexdigest()
        password_list.add((password, sha3_password))
...
for complexity in complexities:
    for length in password_lengths:
        clear_file_name = f'{complexity}_len{length}_clear_SHA3.txt'
        hash_file_name = f'{complexity}_len{length}_hashes_SHA3.txt'
